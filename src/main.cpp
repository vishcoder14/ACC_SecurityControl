...
void setup() {
  ...
  digitalWrite(SMPS_TRIGG_RPIN, LOW);
  digitalWrite(CABIN_LIGHT_RPIN, LOW);

  noTone(buzzerPin);
  SPI.begin(); // start SPI communication.
  rfid.PCD_Init(); // initialize the MFRC522.
  delay(750);
  Serial.println("Please scan your RFID Card at the scanner");
}
...
int getID() {
  ...
  /* COMPARES & CHECK AUTHENTICATION UIDs */
  // For SMPS power access authentication [switch ON]:
  if(IsArrayEqual(SMPSAuthUID, scanCardUID, rfid.uid.size)) {
    if(digitalRead(SMPS_TRIGG_RPIN) == LOW) {
      digitalWrite(SMPS_TRIGG_RPIN, HIGH);
      Serial.println("Access Granted: SMPS switched ON");
    }
    else if(digitalRead(SMPS_TRIGG_RPIN) == HIGH) {
      Serial.println("Access Granted: SMPS already switched ON!");
    }
  }

  // For cabin light power access [switch ON]: 
  if(IsArrayEqual(ClightAuthUID, scanCardUID, rfid.uid.size)) {
    if(digitalRead(CABIN_LIGHT_RPIN) == LOW) {
      digitalWrite(CABIN_LIGHT_RPIN, HIGH);
      Serial.println("Access Granted: Cabin light switched ON");
    }
    else if(digitalRead(CABIN_LIGHT_RPIN) == HIGH) {
      Serial.println("Access Granted: Cabin light already switched ON!");
    }
  }
